...
void insertion_sort_string(char* v[],int n){
    ...
    for(int i=1;i<n;i++){
        new_value = v[i];
        for(int j=i;j>=1 && (strcmp(v[j-1],new_value)>0);j--){
            v[j] = v[j-1];
        }
        v[i] = new_value;
    }
...
int pesquisaBinaria(char* v[],int n,char* procurado){
    int inicio = 0;
    int fim = n-1;
    int meio;
...
int main()
{
  ...
  // Regista o tempo antes e depois da ordenação e pesquisa para medir a duração da ordenação
  ini = clock();
  char procurado[100];

  puts("Sorting: start . . .");
  insertion_sort_string(nomes,n_nomes);
  puts("Sorting: done!");
  ...
  int resPesq = pesquisaBinaria(nomes,n_nomes,procurado);
  puts("Sorting: done!");
  fim = clock();
